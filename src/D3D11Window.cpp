...
WCHAR* CStringToWCHARString(const char* cString)
{
	int length = strlen(cString);
	WCHAR* wcharString = new WCHAR[length];
	MultiByteToWideChar(CP_ACP, 0, cString, -1, wcharString, length);
	return wcharString;
}


bool GetWindow(HWND* windowHandleOut, WindowParams params)   
{
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE /*hPrevInstance*/, LPSTR /*lpCmdLine*/, int /*nShowCmd*/)
{
	...
	HWND hwnd;
	if (!GetWindow(&hwnd, windowParams))
	{
